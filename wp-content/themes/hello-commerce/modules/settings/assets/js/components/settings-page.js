				...
				<Box sx={ { width: '100%' } }>
					<Stack justifyContent={ 'space-between' } direction={ 'row' } alignItems={ 'center' }>
						<Typography variant="h4" gutterBottom>
							{ __( 'Advanced theme settings', 'hello-commerce' ) }
						</Typography>
						<Link href="#" onClick={ ( event ) => handleOpen( event ) } color={ 'primary' }>
							{ __( 'Changelog', 'hello-commerce' ) }
						</Link>
					</Stack>
					<Typography variant="body2" component="div" sx={ { mb: 4 } }>
						{ __( 'Advanced settings are available for experienced users and developers. If you\'re unsure about a setting, we recommend keeping the default option.', 'hello-commerce' ) }
					</Typography>
					<Box>
						<Notices />
					</Box>
					<Box sx={ { borderBottom: 1, borderColor: 'divider' } }>
						<StyledTabs { ...getTabsProps() } aria-label="basic tabs example">
							<StyledTab label={ __( 'SEO and accessibility', 'hello-commerce' ) } { ...getTabProps( 'one' ) } />
							<StyledTab label={ __( 'Structure and layout', 'hello-commerce' ) } { ...getTabProps( 'two' ) } />
							<StyledTab label={ __( 'CSS and styling control', 'hello-commerce' ) } { ...getTabProps( 'three' ) } />
						</StyledTabs>
					</Box>
